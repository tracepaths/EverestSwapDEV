...
(async()=>{
  ...
  const poolBc=(await rpc('octra_compileAml',[fs.readFileSync(path.join(__dirname,'..','contracts','SwapPool.aml'),'utf-8')])).result.bytecode;
  ...
  {N.v+=1;let ts=Date.now()/1000;if(ts%1===0)ts+=1e-6;const tx={from:me,to_:probe,amount:'0',nonce:N.v,ou:'1000',timestamp:ts,op_type:'call',encrypted_data:'set_tmpl',message:JSON.stringify([poolBc])};sign(tx,sk);const r=await rpc('octra_submit',[tx]);const rc=await rec(r.result.tx_hash);console.log('set_tmpl(SwapPool) success=',rc&&rc.success);}
  console.log('do_spawn(SwapPool)...');
  {N.v+=1;let ts=Date.now()/1000;if(ts%1===0)ts+=1e-6;const tx={from:me,to_:probe,amount:'0',nonce:N.v,ou:'1000',timestamp:ts,op_type:'call',encrypted_data:'do_spawn',message:'[]'};sign(tx,sk);const r=await rpc('octra_submit',[tx]);console.log('  submit:',JSON.stringify(r.result||r.error));const rc=await rec(r.result.tx_hash);console.log('  receipt:',rc?('success='+rc.success+' err='+rc.error+' effort='+rc.effort):'NONE');}
  const cnt=await rpc('contract_call',[probe,'get_count',[]]);console.log('  get_count:',cnt.result&&(cnt.result.result??cnt.result));
