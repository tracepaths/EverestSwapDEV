// Signing and the raw RPC transport. Everything that has to be byte-exact about
// a transaction lives here and nowhere else, because the failures it prevents are
// silent ones: a timestamp sent as an integer instead of a float, or a field
// serialised in the wrong order, produces a signature the node simply rejects
// with no indication of which field was wrong.
//
// Nothing here knows what a pool is. The layer above (octra-chain.mjs) builds
// deploys and calls on top of it, and addresses come from deployments-cl.json —
// never from constants in this file, which is how a script ends up quietly
// operating on a retired deployment.
...
export async function rpcCall(method, params, timeoutMs = DEFAULT_RPC_CALL_TIMEOUT_MS) {
  ...
  clearTimeout(timer);
  // The public node returns an HTML error page (not JSON) when rate-limited,
  // so read as text first and surface a diagnosable message.
  const text = await res.text();
  let data;
  try {
    data = JSON.parse(text);
  } catch {
    throw new Error(`RPC ${method}: non-JSON response (HTTP ${res.status}): ${text.slice(0, 120).replace(/\s+/g, ' ')}`);
  }
...
// alongside the receipt: a revert or a bad-bytecode rejection surfaces in seconds
// with the actual cause instead of after a ten-minute timeout with none.
export async function txStatus(txHash, timeoutMs = DEFAULT_RPC_CALL_TIMEOUT_MS) {
  try {
    return await rpcCall('octra_transaction', [txHash], timeoutMs);
  } catch {
    return null;
  }
}
...
function rejectionOf(t) {
  ...
  const e = t.error || {};
  return `${e.type || 'rejected'}: ${e.reason || JSON.stringify(e)}`;
}
...
export async function waitReceipt(txHash, label = 'tx', options = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    const why = rejectionOf(await txStatus(txHash, rpcTimeoutMs));
    if (why) throw new Error(`${ON_CHAIN_FAILURE}: ${why}`);
    await new Promise((res) => setTimeout(res, RECEIPT_POLL_MS));
  }
  const why = rejectionOf(await txStatus(txHash, rpcTimeoutMs));
  throw new Error(
    why
      ? `${ON_CHAIN_FAILURE}: ${why}`
