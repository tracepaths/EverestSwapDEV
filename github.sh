...
# Extract PAT from an existing remote URL. Accepts both the bare form
# (https://<token>@github.com) and the explicit-username form this script now
# writes (https://x-access-token:<token>@github.com), returning only the token.
# Without the colon handling, re-reading a URL this script wrote would hand back
# "x-access-token:<token>" and the next write would nest it twice.
extract_token_from_remote() {
    local remote_url userinfo
...
#
# The token goes in as the *password* behind an x-access-token username rather
# than as a bare userinfo field. Git reads "https://<token>@host" as a username
# with no password, then blocks asking for the password — which with no terminal
# fails outright as "could not read Password". Naming the username explicitly
# leaves the token where git expects a password, so it authenticates unattended.
set_remote_with_token() {
    local clean_url="$1"
    local token="$2"
    # Built by substring rather than sed: a token is arbitrary text and would
    # otherwise need escaping against sed's delimiter and backreferences.
    git remote set-url origin "https://x-access-token:${token}@${clean_url#https://}"
}
